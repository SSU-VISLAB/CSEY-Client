  ...
  return (
    <Wrapper>
      {settingContents.map((data, i) => (
        <PaperBox color={''} key={i} header={data.header}>
          <SettingList content={data.contents}></SettingList>
        </PaperBox>
      ))}
    </Wrapper>
  );
  ...
  return (
    <Wrapper>
      ...
      {alarmGroups?.map((alarmGroup, i) => (
        <AlarmSettingGroup key={i} group={alarmGroup}></AlarmSettingGroup>
      ))}
    </Wrapper>
  );
};

const AlarmSettingGroup = memo(
  ({ group }: { group: AlarmGroup }) => {
    ...
    return (
      <Group elevation={4}>
        <ItemHeader title={header.title} bold />
        <ContentGroup>
          {contents.map((v) => {
            if (v.hasCheck)
              return (
  ...
  (prev, next) => {
    ...
    return prev.group.contents.every(
      (content, index) => content.value == nextContents[index].value,
    );
  },
);
